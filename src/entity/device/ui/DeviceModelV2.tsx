...
export default function DeviceModel3D({
    ...
    const panelMargin = Math.min(width) * 0.04; // 8% 패딩
    const panelWidth = width - panelMargin * 2;
    const panelHeight = height - panelMargin * 2;
    const panelRadius = Math.min(panelWidth, panelHeight) * 0.1; // 패널 둥근 모서리 반경
    ...
    return (
        <group ref={groupRef}>
            ...
            <mesh
                position={[
                    0,                          // X: 중앙
                    height * 0.1,               // Y: 약간 위 (높이의 20% 위치)
                    depth / 2 + 0.001           // Z: 패널보다 확실히 앞으로
                ]}
                rotation={[0, 0, 0]}
                renderOrder={2}
            >
                <planeGeometry args={[panelWidth * 0.7, panelHeight * 0.7]} />
